...
FEATURES_BASE_DIR = "features/cnn-32-frame/" # NEW: Separate folder for these features/models
os.makedirs(FEATURES_BASE_DIR, exist_ok=True)

# !!! IMPORTANT: Update these paths to your actual 32-frame feature files !!!
X_TRAIN_FEATURES_FILE = f"features/cnn-32-frame/X_train_sequences_{percentage_str}pct.npy" # EXAMPLE PATH
...
MODEL_SAVE_PATH = os.path.join(FEATURES_BASE_DIR, f"best_lstm_model_{percentage_str}pct.pth")
METRICS_CSV_PATH = os.path.join(FEATURES_BASE_DIR, f"training_metrics_lstm_{percentage_str}pct.csv")
PLOT_FILENAME = os.path.join(FEATURES_BASE_DIR, f"training_curves_lstm_{percentage_str}pct.png")
HISTORY_FILENAME = os.path.join(FEATURES_BASE_DIR, f"training_history_lstm_{percentage_str}pct.npy")
...
if os.path.exists(X_TRAIN_FEATURES_FILE) and os.path.exists(X_TEST_FEATURES_FILE):
    ...
    print(f"Training sequences shape: {X_train_sequences.shape}") # Expected: (num_videos, NUM_FRAMES, FEAT_DIM)
...
class LSTMCollisionClassifier(nn.Module):
    def __init__(self, input_dim=FEAT_DIM, lstm_hidden_dim=LSTM_HIDDEN_DIM,
                 mlp_hidden_dims=None, dropout=DROPOUT_RATE, num_lstm_layers=1):
        ...
        self.lstm = nn.LSTM(input_size=input_dim,
                              ...
                              dropout=dropout if num_lstm_layers > 1 else 0, # add dropout between LSTM layers if multiple
                              bidirectional=True) # Starting with unidirectional
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch + 1) % PRINT_FREQUENCY == 0:
        print(f"Epoch {epoch+1}/{NUM_EPOCHS} | LR: {current_lr:.1e} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val AUC: {val_auc:.4f} | Best Val AUC: {best_val_auc:.4f}")
...
submission_filename = os.path.join(FEATURES_BASE_DIR, f"submission_lstm_{percentage_str}pct.csv")
...
history_data = {
    ...
    "learning_rates": learning_rates, "best_val_auc": best_val_auc, "final_val_auc": final_val_auc,
