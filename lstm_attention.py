...
X_TEST_FEATURES_FILE = "features/cnn-32-frame/X_test_sequences.npy"      # Using existing 32-frame features
SCALER_FILE = os.path.join(FEATURES_BASE_DIR, f"scaler_lstm_attention_{percentage_str}pct.joblib")

MODEL_SAVE_PATH = os.path.join(FEATURES_BASE_DIR, f"best_lstm_attention_model_{percentage_str}pct.pth")
METRICS_CSV_PATH = os.path.join(FEATURES_BASE_DIR, f"training_metrics_lstm_attention_{percentage_str}pct.csv")
PLOT_FILENAME = os.path.join(FEATURES_BASE_DIR, f"training_curves_lstm_attention_{percentage_str}pct.png")
HISTORY_FILENAME = os.path.join(FEATURES_BASE_DIR, f"training_history_lstm_attention_{percentage_str}pct.npy")
...
if X_train_sequences.shape[1] != NUM_FRAMES:
    print(f"Warning: Loaded training features have {X_train_sequences.shape[1]} frames, but NUM_FRAMES is {NUM_FRAMES}.") #
if X_train_sequences.shape[2] != FEAT_DIM:
    print(f"Warning: Loaded training features have dimension {X_train_sequences.shape[2]}, but FEAT_DIM is {FEAT_DIM}.") #
...
class AttentionModule(nn.Module):
    ...
    def forward(self, lstm_outputs): # lstm_outputs: (batch, seq_len, lstm_output_dim)
        ...
        return context, alpha # Return context vector and attention weights (for inspection)

# --- MODIFIED: LSTM Classifier with Attention ---
class LSTMAttentionClassifier(nn.Module):
    def __init__(self, input_dim=FEAT_DIM, lstm_hidden_dim=LSTM_HIDDEN_DIM,
                 attention_dim=ATTENTION_DIM, mlp_hidden_dims=None,
                 dropout=DROPOUT_RATE, num_lstm_layers=NUM_LSTM_LAYERS): # num_lstm_layers was hardcoded to 1 before
        ...
        lstm_output_dim = lstm_hidden_dim * self.num_directions # (derived, as prev_mlp_dim = lstm_hidden_dim * 2)
    ...
    def forward(self, x_sequence): # x_sequence: (batch_size, seq_len, feature_dim)
        ...
        output = self.classifier_mlp(context_vector) # (context_vector replaces last_time_step_output)
        return output
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", patience=LR_PATIENCE, factor=LR_FACTOR) # (changed from val_auc to val_loss if mode is min)
...
def train_epoch_model(model, train_loader, criterion, optimizer, device): # Renamed for clarity
    model.train() #
    total_loss = 0 #
...
def validate_model(model, val_loader, criterion, device): # Renamed for clarity
    ...
    if len(np.unique(all_true_labels_np)) < 2: #
        auc_score = 0.5 #
        print(f"Warning: Only one class present in validation set for this epoch. AUC set to {auc_score}.") #
    else:
...
for epoch in range(NUM_EPOCHS): #
    train_loss = train_epoch_model(model, train_loader_seq, criterion, optimizer, device) #
    ...
    if (epoch + 1) % PRINT_FREQUENCY == 0: #
        print(f"Epoch {epoch+1}/{NUM_EPOCHS} | LR: {current_lr:.1e} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val AUC: {val_auc:.4f} | Best Val AUC for Saving: {best_val_auc_for_saving:.4f}") #
...
final_val_loss, final_val_auc = validate_model(model, val_loader_seq, criterion, device) #
...
test_dataset_seq = SequenceDataset(X_test_scaled_sequences, np.zeros(len(X_test_scaled_sequences))) #
...
submission_filename = os.path.join(FEATURES_BASE_DIR, f"submission_lstm_attention_{percentage_str}pct.csv") #
...
history_data = { #
    ...
    "learning_rates": learning_rates, "best_val_auc_for_saving": best_val_auc_for_saving, "final_val_auc": final_val_auc, # (updated key)
    "dataset_percentage": DATASET_PERCENTAGE, "num_frames": NUM_FRAMES #
}
