...
from sklearn.preprocessing import (
    StandardScaler,
)  # For potential future use, not in this script
...
if DATASET_PERCENTAGE < 1.0:
    ...
    ).reset_index(drop=True)  # Using a fixed random state for sampling consistency
else:
...
def calculate_optical_flow(frames_sequence):
    ...
    if len(frames_sequence) < 2:
        return np.empty(
            (0, *frames_sequence.shape[1:3], 2), dtype=np.float32
        )  # H, W, 2 for flow
...
def get_optical_flow_feature_sequences(
    ...
    for video_id in tqdm(
        ...
        if (
            not batch_flow_cnn_inputs
        ):  # Should not happen if current_num_flow_frames > 0
            print(
        ...
        if current_num_flow_frames < target_num_flow_frames:
            ...
            current_num_flow_frames > target_num_flow_frames
...
if os.path.exists(X_TRAIN_FLOW_FEATURES_FILE) and os.path.exists(
    ...
    if X_train_flow_sequences.size > 0:
        ...
        print(
            f"Training flow sequences shape: {X_train_flow_sequences.shape}"
        )  # (num_videos, NUM_FRAMES-1, FLOW_FEAT_DIM)
    else:
    ...
    if X_test_flow_sequences.size > 0:
        ...
        print(
            f"Test flow sequences shape: {X_test_flow_sequences.shape}"
        )  # (num_videos, NUM_FRAMES-1, FLOW_FEAT_DIM)
    else:
