...
def extract_rgb_frames(video_path, num_frames=NUM_FRAMES_TO_EXTRACT, size=FRAME_SIZE):
    ...
    if total_video_frames <= 0:
        cap.release(); print(f"Warning: Could not read video or video is empty: {video_path}"); return np.empty((0, *size, 3), dtype=np.uint8)
    if total_video_frames < num_frames: frames_to_extract_indices = np.arange(0, total_video_frames)
    else: frames_to_extract_indices = np.linspace(0, total_video_frames - 1, num_frames, dtype=int)
    extracted_frames = []
    for idx in frames_to_extract_indices:
        cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
        ret, frame = cap.read()
        if not ret: print(f"Warning: Could not read frame at index {idx} from {video_path}"); break
        frame = cv2.resize(frame, size); extracted_frames.append(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
    cap.release()
    if not extracted_frames: print(f"Warning: No frames extracted for video: {video_path}"); return np.empty((0, *size, 3), dtype=np.uint8)
...
def calculate_optical_flow(frames_sequence):
    ...
    flow_sequence = []
    if len(frames_sequence) < 2: print("Warning: Need at least 2 frames to calculate optical flow."); return np.empty((0, *frames_sequence.shape[1:3], 2), dtype=np.float32)
    ...
    for i in range(1, len(frames_sequence)):
        ...
        flow_sequence.append(flow); prev_gray = current_gray
...
def visualize_optical_flow_arrows(rgb_frame, flow_field, frame_index_str="", step=16):
    ...
    plt.quiver(x, y, fx, fy, color='red', angles='xy', scale_units='xy', scale=1, headwidth=3, headlength=4, width=0.002)
    ...
    plt.title(f"Optical Flow (Arrows) - Frame {frame_index_str} to {int(frame_index_str)+1 if frame_index_str.isdigit() else ''}")
    plt.axis('off')
...
def main_visualize_video_flow_arrows(video_id, video_dir):
    ...
    if rgb_frames.shape[0] < 2: print("Not enough RGB frames for optical flow."); return
    print(f"Extracted {rgb_frames.shape[0]} RGB frames.")

    optical_flow_fields = calculate_optical_flow(rgb_frames)
    if optical_flow_fields.shape[0] == 0: print("No optical flow fields generated."); return
    ...
    if num_to_show == 0: print("Nothing to display."); return
    ...
    for i in range(num_to_show):
        ...
        visualize_optical_flow_arrows(base_rgb_frame, flow_field_to_show, frame_index_str=str(i), step=10) # Adjust step for density
